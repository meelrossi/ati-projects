...
public class PixelExchangeBorder {
	...
	private boolean isInLOut(Pixel pixel) {
		for (Pixel neigh : neighbours(pixel)) {
			if (neigh.getType().equals(PixelType.IN) || neigh.getType().equals(PixelType.BORDER_IN)) {
				return true;
			}
		}
		return false;
	...
	private boolean isInLIn(Pixel pixel) {
		for (Pixel neigh : neighbours(pixel)) {
			if (neigh.getType().equals(PixelType.OUT) || neigh.getType().equals(PixelType.BORDER_OUT)) {
				return true;
			}
		}
		return false;
	...
	private void initializeData(int initialX, int initialY, int rectWidth, int rectHeight) {
		...
		outsideColor = Color.rgb((int) outColor[0] / outCount, (int) outColor[1] / outCount,
				(int) outColor[2] / outCount);

	}

	private boolean isInternal(int x, int y, int initialX, int initialY, int rectWidth, int rectHeight) {
		return (x >= initialX + 2 && x <= initialX + rectWidth - 2)
				&& (y >= initialY + 2 && y <= initialY + rectHeight - 2);
	}

	private boolean isInternalBorder(int x, int y, int initialX, int initialY, int rectWidth, int rectHeight) {
		if ((x == initialX + 1 || x == initialX + rectWidth - 1)
				&& (y >= initialY + 1 && y <= initialY + rectHeight - 1)) {
			return true;
		}
		if ((y == initialY + 1 || y == initialY + rectHeight - 1)
				&& (x >= initialX + 1 && x <= initialX + rectWidth - 1)) {
			return true;
		}
		return false;
	}
	...
	private double ForceS(int x, int y) {
		...
		for (int i = x - move; i <= x + move; i++) {
			for (int j = y - move; j <= y + move; j++) {
				if (i >= 0 && i < currentImage.getWidth() && j >= 0 && j < currentImage.getHeight()) {
					int dx = Math.abs(x - i);
					int dy = Math.abs(y - j);
